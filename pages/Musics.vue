<template>
  <div class="app-index">
    <div class="app-wrapper">
      <div class="app-left">
        <!-- Music List -->
        <app-group v-for="(list, $index) in musicList" :key="$index" :type="$index % 2 === 0 ? 'left' : 'right'" :list="list"></app-group>
...
export default {
  ...
  methods: {
    ...
    handleLoadMore () {
      this.currentPage += 1
      this.getMusicList({page: this.currentPage, limit: this.limit})
    }
  },
  created () {
    this.getMusicList({page: this.currentPage, limit: this.limit})
